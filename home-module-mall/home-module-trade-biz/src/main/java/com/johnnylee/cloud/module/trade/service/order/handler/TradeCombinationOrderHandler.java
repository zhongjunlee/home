...
public class TradeCombinationOrderHandler implements TradeOrderHandler {
    ...
    public void beforeOrderCreate(TradeOrderDO order, List<TradeOrderItemDO> orderItems) {
        ...
        if (activityOrder != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.ORDER_CREATE_FAIL_EXIST_UNPAID);
        }
    }
    ...
    public void beforeDeliveryOrder(TradeOrderDO order) {
        ...
        if (!combinationRecordApi.isCombinationRecordSuccess(order.getUserId(), order.getId()).getCheckedData()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.ORDER_DELIVERY_FAIL_COMBINATION_RECORD_STATUS_NOT_SUCCESS);
        }
    }
