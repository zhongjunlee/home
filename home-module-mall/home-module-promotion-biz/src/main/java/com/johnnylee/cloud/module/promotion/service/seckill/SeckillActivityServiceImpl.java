package com.johnnylee.cloud.module.promotion.service.seckill;
...
import com.johnnylee.cloud.module.promotion.controller.admin.seckill.vo.activity.SeckillActivityPageReqVO;
...
import com.johnnylee.cloud.module.promotion.controller.app.seckill.vo.activity.AppSeckillActivityPageReqVO;
import com.johnnylee.cloud.module.promotion.convert.seckill.seckillactivity.SeckillActivityConvert;
...
import com.johnnylee.cloud.module.promotion.dal.mysql.seckill.seckillactivity.SeckillActivityMapper;
import com.johnnylee.cloud.module.promotion.dal.mysql.seckill.seckillactivity.SeckillProductMapper;
...
import static com.johnnylee.cloud.framework.common.util.date.LocalDateTimeUtils.isBetween;
...
public class SeckillActivityServiceImpl implements SeckillActivityService {
    ...
    private void validateProductConflict(List<Long> configIds, Long spuId, Long activityId) {
        ...
        if (isNotEmpty(conflictActivityList)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_ACTIVITY_SPU_CONFLICTS);
        }
    }
    ...
    public void updateSeckillActivity(SeckillActivityUpdateReqVO updateReqVO) {
        ...
        if (CommonStatusEnum.DISABLE.getStatus().equals(activity.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_ACTIVITY_UPDATE_FAIL_STATUS_CLOSED);
        }
        // 1.2 校验商品是否冲突
    ...
    public void updateSeckillStockDecr(Long id, Long skuId, Integer count) {
        ...
        if (count > seckillActivity.getTotalStock()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_ACTIVITY_UPDATE_STOCK_FAIL);
        }
        // 1.2 校验商品库存是否充足
    ...
    public void closeSeckillActivity(Long id) {
        ...
        if (CommonStatusEnum.DISABLE.getStatus().equals(activity.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_ACTIVITY_CLOSE_FAIL_STATUS_CLOSED);
        }
    ...
    public void deleteSeckillActivity(Long id) {
        ...
        if (CommonStatusEnum.ENABLE.getStatus().equals(seckillActivity.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_ACTIVITY_DELETE_FAIL_STATUS_NOT_CLOSED_OR_END);
        }
    ...
    private SeckillActivityDO validateSeckillActivityExists(Long id) {
        ...
        if (seckillActivity == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_ACTIVITY_NOT_EXISTS);
        }
    ...
    public SeckillValidateJoinRespDTO validateJoinSeckill(Long activityId, Long skuId, Integer count) {
        ...
        if (ObjectUtil.notEqual(activity.getStatus(), CommonStatusEnum.ENABLE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_JOIN_ACTIVITY_STATUS_CLOSED);
        }
        // 1.2 是否在活动时间范围内
        if (!LocalDateTimeUtils.isBetween(activity.getStartTime(), activity.getEndTime())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_JOIN_ACTIVITY_TIME_ERROR);
        }
        ...
        if (count > activity.getSingleLimitCount()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_JOIN_ACTIVITY_SINGLE_LIMIT_COUNT_EXCEED);
        }

        // 2.1 校验秒杀商品是否存在
        SeckillProductDO product = seckillProductMapper.selectByActivityIdAndSkuId(activityId, skuId);
        if (product == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_JOIN_ACTIVITY_PRODUCT_NOT_EXISTS);
        }
        // 2.2 校验库存是否充足
