...
import com.johnnylee.cloud.module.promotion.controller.admin.bargain.vo.recrod.BargainRecordPageReqVO;
import com.johnnylee.cloud.module.promotion.controller.app.bargain.vo.record.AppBargainRecordCreateReqVO;
...
import com.johnnylee.cloud.module.promotion.dal.mysql.bargain.BargainRecordMapper;
...
public class BargainRecordServiceImpl implements BargainRecordService {
    ...
    public Long createBargainRecord(Long userId, AppBargainRecordCreateReqVO reqVO) {
        ...
        if (CollUtil.isNotEmpty(bargainRecordMapper.selectListByUserIdAndActivityIdAndStatus(
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_RECORD_CREATE_FAIL_EXISTS);
        }
        // 2.2 是否超过参与的上限
        if (bargainRecordMapper.selectCountByUserIdAndActivityIdAndStatus(
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_RECORD_CREATE_FAIL_LIMIT);
        }
    ...
    public BargainValidateJoinRespDTO validateJoinBargain(Long userId, Long bargainRecordId, Long skuId) {
        ...
        if (ObjUtil.notEqual(record.getStatus(), BargainRecordStatusEnum.SUCCESS.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_JOIN_RECORD_NOT_SUCCESS);
        }
        // 1.3 砍价记录已经下单
        if (record.getOrderId() != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_JOIN_RECORD_ALREADY_ORDER);
        }
