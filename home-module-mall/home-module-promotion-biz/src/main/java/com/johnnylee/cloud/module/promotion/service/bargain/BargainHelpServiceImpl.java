...
import com.johnnylee.cloud.module.promotion.controller.app.bargain.vo.help.AppBargainHelpCreateReqVO;
...
import com.johnnylee.cloud.module.promotion.dal.mysql.bargain.BargainHelpMapper;
...
public class BargainHelpServiceImpl implements BargainHelpService {
    ...
    public BargainHelpDO createBargainHelp(Long userId, AppBargainHelpCreateReqVO reqVO) {
        ...
        if (record == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_RECORD_NOT_EXISTS);
        }
        if (ObjUtil.notEqual(record.getStatus(), BargainRecordStatusEnum.IN_PROGRESS.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_HELP_CREATE_FAIL_RECORD_NOT_IN_PROCESS);
        }
        // 1.2 不能自己给自己砍价
        if (ObjUtil.equal(record.getUserId(), userId)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_HELP_CREATE_FAIL_RECORD_SELF);
        }
        ...
        if (bargainHelpMapper.selectCountByUserIdAndActivityId(userId, activity.getId())
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_HELP_CREATE_FAIL_LIMIT);
        }
        ...
        if (bargainHelpMapper.selectByUserIdAndRecordId(userId, record.getId()) != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_HELP_CREATE_FAIL_HELP_EXISTS);
        }
        ...
        if (!bargainRecordService.updateBargainRecordBargainPrice(
            ...
            // 多人一起砍价，需要重试
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.BARGAIN_HELP_CREATE_FAIL_CONFLICT);
        }
        return help;
    }
