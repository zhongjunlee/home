package com.johnnylee.cloud.module.promotion.service.coupon;
...
import com.johnnylee.cloud.module.promotion.controller.app.coupon.vo.coupon.AppCouponMatchReqVO;
...
import com.johnnylee.cloud.module.promotion.dal.mysql.coupon.CouponMapper;
...
public class CouponServiceImpl implements CouponService {
    ...
    public CouponDO validCoupon(Long id, Long userId) {
        ...
        if (coupon == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_NOT_EXISTS);
        }
        validCoupon(coupon);
        return coupon;
    }
    ...
    public void validCoupon(CouponDO coupon) {
        ...
        if (ObjectUtil.notEqual(coupon.getStatus(), CouponStatusEnum.UNUSED.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_STATUS_NOT_UNUSED);
        }
        // 校验有效期；为避免定时器没跑，实际优惠劵已经过期
        if (LocalDateTimeUtils.isBetween(coupon.getValidStartTime(), coupon.getValidEndTime())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_VALID_TIME_NOT_NOW);
        }
    }
    ...
    public void returnUsedCoupon(Long id) {
        ...
        if (ObjectUtil.notEqual(coupon.getTemplateId(), CouponStatusEnum.USED.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_STATUS_NOT_USED);
        }
    ...
    public void deleteCoupon(Long id) {
        ...
        if (deleteCount == 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_DELETE_FAIL_USED);
        }
        // 减少优惠劵模板的领取数量 -1
    ...
    private void validateCouponTemplateCanTake(CouponTemplateDO couponTemplate, Set<Long> userIds, CouponTakeTypeEnum takeType) {
        ...
        if (CollUtil.isEmpty(userIds)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_TEMPLATE_USER_ALREADY_TAKE);
        }
        ...
        if (couponTemplate.getTakeCount() + userIds.size() > couponTemplate.getTotalCount()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_TEMPLATE_NOT_ENOUGH);
        }
        // 校验"固定日期"的有效期类型是否过期
        if (CouponTemplateValidityTypeEnum.DATE.getType().equals(couponTemplate.getValidityType())) {
            if (LocalDateTimeUtils.beforeNow(couponTemplate.getValidEndTime())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_TEMPLATE_EXPIRED);
            }
        }
        // 校验领取方式
        if (ObjectUtil.notEqual(couponTemplate.getTakeType(), takeType.getValue())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COUPON_TEMPLATE_CANNOT_TAKE);
        }
    }
