package com.johnnylee.cloud.module.promotion.service.combination;
...
import com.johnnylee.cloud.module.promotion.controller.admin.combination.vo.recrod.CombinationRecordReqPageVO;
...
import com.johnnylee.cloud.module.promotion.dal.mysql.combination.CombinationRecordMapper;
...
import static com.johnnylee.cloud.framework.common.util.date.LocalDateTimeUtils.afterNow;
import static com.johnnylee.cloud.framework.common.util.date.LocalDateTimeUtils.beforeNow;
...
public class CombinationRecordServiceImpl implements CombinationRecordService {
    ...
    public KeyValue<CombinationActivityDO, CombinationProductDO> validateCombinationRecord(
        ...
        if (ObjUtil.equal(activity.getStatus(), CommonStatusEnum.DISABLE.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_ACTIVITY_STATUS_DISABLE);
        }
        // 1.2. 校验活动开始时间
        if (afterNow(activity.getStartTime())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_RECORD_FAILED_TIME_NOT_START);
        }
        // 1.3 校验是否超出单次限购数量
        if (count > activity.getSingleLimitCount()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_RECORD_FAILED_SINGLE_LIMIT_COUNT_EXCEED);
        }
        ...
        if (headId != null) {
            ...
            if (record == null) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_RECORD_HEAD_NOT_EXISTS);
            }
            // 2.2. 校验拼团是否已满
            if (ObjUtil.equal(record.getUserCount(), record.getUserSize())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_RECORD_USER_FULL);
            }
            // 2.3 校验拼团是否过期（有父拼团的时候只校验父拼团的过期时间）
            if (beforeNow(record.getExpireTime())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_RECORD_FAILED_TIME_END);
            }
        } else {
        ...
        if (product == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_JOIN_ACTIVITY_PRODUCT_NOT_EXISTS);
        }
        // 4.2 校验 sku 是否存在
        ...
        if (count > sku.getStock()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_ACTIVITY_UPDATE_STOCK_FAIL);
        }
        ...
        if (inProgressRecord != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_RECORD_FAILED_HAVE_JOINED);
        }
        // 6.3 校验是否超出总限购数量
        ...
        if (sumValue != null && sumValue + count > activity.getTotalLimitCount()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.COMBINATION_RECORD_FAILED_TOTAL_LIMIT_COUNT_EXCEED);
        }
        return new KeyValue<>(activity, product);
    }
