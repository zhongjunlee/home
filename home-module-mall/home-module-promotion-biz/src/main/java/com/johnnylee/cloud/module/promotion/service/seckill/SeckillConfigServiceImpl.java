...
import com.johnnylee.cloud.module.promotion.controller.admin.seckill.vo.config.SeckillConfigPageReqVO;
...
import com.johnnylee.cloud.module.promotion.dal.mysql.seckill.seckillconfig.SeckillConfigMapper;
...
import static com.johnnylee.cloud.framework.common.util.collection.CollectionUtils.findFirst;
...
public class SeckillConfigServiceImpl implements SeckillConfigService {
    ...
    private void validateSeckillConfigExists(Long id) {
        if (seckillConfigMapper.selectById(id) == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_CONFIG_NOT_EXISTS);
        }
    }
    ...
    private void validateSeckillConfigConflict(String startTimeStr, String endTimeStr, Long id) {
        ...
        if (hasConflict) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_CONFIG_TIME_CONFLICTS);
        }
    }
    ...
    public void validateSeckillConfigExists(Collection<Long> ids) {
        ...
        configs.forEach(config -> {
            if (ObjectUtil.equal(config.getStatus(), CommonStatusEnum.DISABLE.getStatus())) {
                throw ServiceExceptionUtil.exception(ErrorCodeConstants.SECKILL_CONFIG_DISABLE);
            }
        });
    }
