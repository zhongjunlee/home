package com.johnnylee.cloud.module.promotion.service.discount;
...
import com.johnnylee.cloud.module.promotion.controller.admin.discount.vo.DiscountActivityBaseVO;
import com.johnnylee.cloud.module.promotion.controller.admin.discount.vo.DiscountActivityCreateReqVO;
import com.johnnylee.cloud.module.promotion.controller.admin.discount.vo.DiscountActivityPageReqVO;
import com.johnnylee.cloud.module.promotion.controller.admin.discount.vo.DiscountActivityUpdateReqVO;
...
import com.johnnylee.cloud.module.promotion.dal.dataobject.discount.DiscountProductDO;
import com.johnnylee.cloud.module.promotion.dal.mysql.discount.DiscountActivityMapper;
import com.johnnylee.cloud.module.promotion.dal.mysql.discount.DiscountProductMapper;
import com.johnnylee.cloud.module.promotion.enums.common.PromotionActivityStatusEnum;
import com.johnnylee.cloud.module.promotion.util.PromotionUtils;
...
public class DiscountActivityServiceImpl implements DiscountActivityService {
    ...
    public void updateDiscountActivity(DiscountActivityUpdateReqVO updateReqVO) {
        ...
        if (discountActivity.getStatus().equals(CommonStatusEnum.DISABLE.getStatus())) { // 已关闭的活动，不能修改噢
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DISCOUNT_ACTIVITY_UPDATE_FAIL_STATUS_CLOSED);
        }
        // 校验商品是否冲突
    ...
    private void validateDiscountActivityProductConflicts(Long id, List<DiscountActivityBaseVO.Product> products) {
        ...
        if (CollUtil.isNotEmpty(matchDiscountProductList)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DISCOUNT_ACTIVITY_SPU_CONFLICTS);
        }
    }
    ...
    public void closeDiscountActivity(Long id) {
        ...
        if (activity.getStatus().equals(CommonStatusEnum.DISABLE.getStatus())) { // 已关闭的活动，不能关闭噢
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DISCOUNT_ACTIVITY_CLOSE_FAIL_STATUS_CLOSED);
        }
    ...
    public void deleteDiscountActivity(Long id) {
        ...
        if (CommonStatusEnum.isEnable(activity.getStatus())) { // 未关闭的活动，不能删除噢
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DISCOUNT_ACTIVITY_DELETE_FAIL_STATUS_NOT_CLOSED);
        }
    ...
    private DiscountActivityDO validateDiscountActivityExists(Long id) {
        ...
        if (discountActivity == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DISCOUNT_ACTIVITY_NOT_EXISTS);
        }
