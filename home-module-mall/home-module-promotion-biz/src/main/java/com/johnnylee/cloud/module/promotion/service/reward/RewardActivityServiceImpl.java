package com.johnnylee.cloud.module.promotion.service.reward;
...
import com.johnnylee.cloud.module.promotion.convert.reward.RewardActivityConvert;
import com.johnnylee.cloud.module.promotion.dal.dataobject.reward.RewardActivityDO;
import com.johnnylee.cloud.module.promotion.dal.mysql.reward.RewardActivityMapper;
...
public class RewardActivityServiceImpl implements RewardActivityService {
    ...
    public void updateRewardActivity(RewardActivityUpdateReqVO updateReqVO) {
        ...
        if (dbRewardActivity.getStatus().equals(PromotionActivityStatusEnum.CLOSE.getStatus())) { // 已关闭的活动，不能修改噢
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REWARD_ACTIVITY_UPDATE_FAIL_STATUS_CLOSED);
        }
        // 校验商品是否冲突
    ...
    public void closeRewardActivity(Long id) {
        ...
        if (dbRewardActivity.getStatus().equals(PromotionActivityStatusEnum.CLOSE.getStatus())) { // 已关闭的活动，不能关闭噢
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REWARD_ACTIVITY_CLOSE_FAIL_STATUS_CLOSED);
        }
        if (dbRewardActivity.getStatus().equals(PromotionActivityStatusEnum.END.getStatus())) { // 已关闭的活动，不能关闭噢
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REWARD_ACTIVITY_CLOSE_FAIL_STATUS_END);
        }
    ...
    public void deleteRewardActivity(Long id) {
        ...
        if (!dbRewardActivity.getStatus().equals(PromotionActivityStatusEnum.CLOSE.getStatus())) { // 未关闭的活动，不能删除噢
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REWARD_ACTIVITY_DELETE_FAIL_STATUS_NOT_CLOSED);
        }
    ...
    private RewardActivityDO validateRewardActivityExists(Long id) {
        ...
        if (activity == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REWARD_ACTIVITY_NOT_EXISTS);
        }
        return activity;
    }
    ...
    private void validateRewardActivitySpuConflicts(Long id, Collection<Long> spuIds) {
        ...
        if (CollUtil.isNotEmpty(rewardActivityList)) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REWARD_ACTIVITY_SPU_CONFLICTS);
        }
    }
