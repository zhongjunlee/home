package com.johnnylee.cloud.module.pay.service.demo;
...
import com.johnnylee.cloud.module.pay.dal.mysql.demo.PayDemoOrderMapper;
...
public class PayDemoOrderServiceImpl implements PayDemoOrderService {
    ...
    public void updateDemoOrderPaid(Long id, Long payOrderId) {
        ...
        if (updateCount == 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_UPDATE_PAID_STATUS_NOT_UNPAID);
        }
    }
    ...
    private PayOrderRespDTO validateDemoOrderCanPaid(Long id, Long payOrderId) {
        ...
        if (order == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_NOT_FOUND);
        }
        // 1.2 校验订单未支付
        ...
        if (notEqual(order.getPayOrderId(), payOrderId)) { // 支付单号
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_UPDATE_PAID_FAIL_PAY_ORDER_ID_ERROR);
        }
        ...
        if (!PayOrderStatusEnum.isSuccess(payOrder.getStatus())) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_UPDATE_PAID_FAIL_PAY_ORDER_STATUS_NOT_SUCCESS);
        }
        // 2.3 校验支付金额一致
        if (notEqual(payOrder.getPrice(), order.getPrice())) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_UPDATE_PAID_FAIL_PAY_PRICE_NOT_MATCH);
        }
        // 2.4 校验支付订单匹配（二次）
    ...
    private PayDemoOrderDO validateDemoOrderCanRefund(Long id) {
        ...
        // 校验订单是否支付
        if (!order.getPayStatus()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_REFUND_FAIL_NOT_PAID);
        }
        // 校验订单是否已退款
        if (order.getPayRefundId() != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_REFUND_FAIL_REFUNDED);
        }
        return order;
    }
    ...
    private PayRefundRespDTO validateDemoOrderCanRefunded(Long id, Long payRefundId) {
        ...
        if (Objects.equals(order.getPayOrderId(), payRefundId)) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_REFUND_FAIL_REFUND_ORDER_ID_ERROR);
        }

        // 2.1 校验退款订单
        PayRefundRespDTO payRefund = payRefundApi.getRefund(payRefundId).getCheckedData();
        if (payRefund == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_REFUND_FAIL_REFUND_NOT_FOUND);
        }
        // 2.2
        if (!PayRefundStatusEnum.isSuccess(payRefund.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_REFUND_FAIL_REFUND_NOT_SUCCESS);
        }
        // 2.3 校验退款金额一致
        if (notEqual(payRefund.getRefundPrice(), order.getPrice())) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.DEMO_ORDER_REFUND_FAIL_REFUND_PRICE_NOT_MATCH);
        }
        // 2.4 校验退款订单匹配（二次）
