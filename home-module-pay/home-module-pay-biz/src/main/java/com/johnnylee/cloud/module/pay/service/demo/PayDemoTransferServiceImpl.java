...
import com.johnnylee.cloud.module.pay.dal.mysql.demo.PayDemoTransferMapper;
...
import static com.johnnylee.cloud.framework.pay.core.enums.transfer.PayTransferTypeEnum.typeOf;
import static com.johnnylee.cloud.module.pay.enums.transfer.PayTransferTypeEnum.*;
        ...
        switch (transferTypeEnum) {
            case ALIPAY_BALANCE: {
                if (StrUtil.isEmpty(MapUtil.getStr(payeeInfo, ALIPAY_LOGON_ID))) {
                    throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_ALIPAY_LOGIN_ID_IS_EMPTY);
                }
                if (StrUtil.isEmpty(MapUtil.getStr(payeeInfo, ALIPAY_ACCOUNT_NAME))) {
                    throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_ALIPAY_ACCOUNT_NAME_IS_EMPTY);
                }
                break;
            }
            case WX_BALANCE:
