...
import com.johnnylee.cloud.module.pay.convert.app.PayAppConvert;
...
public class PayAppServiceImpl implements PayAppService {
    ...
    public void deleteApp(Long id) {
        ...
        if (orderService.getOrderCountByAppId(id) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.APP_EXIST_ORDER_CANT_DELETE);
        }
        if (refundService.getRefundCountByAppId(id) > 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.APP_EXIST_REFUND_CANT_DELETE);
        }
