package com.johnnylee.cloud.module.pay.service.transfer;
...
import com.johnnylee.cloud.framework.pay.core.client.dto.transfer.PayTransferRespDTO;
import com.johnnylee.cloud.framework.pay.core.client.dto.transfer.PayTransferUnifiedReqDTO;
import com.johnnylee.cloud.framework.pay.core.enums.channel.PayChannelEnum;
import com.johnnylee.cloud.framework.pay.core.enums.transfer.PayTransferStatusRespEnum;
import com.johnnylee.cloud.framework.pay.core.enums.transfer.PayTransferTypeEnum;
...
import com.johnnylee.cloud.module.pay.convert.transfer.PayTransferConvert;
...
import com.johnnylee.cloud.module.pay.dal.mysql.transfer.PayTransferExtensionMapper;
import com.johnnylee.cloud.module.pay.dal.mysql.transfer.PayTransferMapper;
import com.johnnylee.cloud.module.pay.dal.redis.no.PayNoRedisDAO;
import com.johnnylee.cloud.module.pay.enums.transfer.PayTransferStatusEnum;
...
public class PayTransferServiceImpl implements PayTransferService {
    ...
    public PayTransferSubmitRespVO submitTransfer(PayTransferSubmitReqVO reqVO, String userIp) {
        ...
        PayTransferExtensionDO transferExtension = new PayTransferExtensionDO().setNo(no)
                ...
                .setChannelCode(channel.getCode()).setStatus(PayTransferStatusEnum.WAITING.getStatus());
        ...
        if (StrUtil.isNotEmpty(unifiedTransferResp.getChannelErrorCode())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_SUBMIT_CHANNEL_ERROR, unifiedTransferResp.getChannelErrorCode(),
    ...
    private Boolean updateTransferSuccess(PayChannelDO channel, PayTransferExtensionDO transferExtension,
        ...
        if (transfer == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_NOT_FOUND);
        }
        if (PayTransferStatusEnum.isSuccess(transfer.getStatus()) && Objects.equals(transfer.getExtensionId(), transferExtension.getId())) {
            log.info("[updateTransferSuccess][transfer({}) 已经是已转账，无需更新]", transfer.getId());
            return true;
        }
        if (!PayTransferStatusEnum.isPendingStatus(transfer.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_STATUS_IS_NOT_PENDING);
        }
        // 2.更新
        int updateCounts = transferMapper.updateByIdAndStatus(transfer.getId(),
                CollUtil.newArrayList(PayTransferStatusEnum.WAITING.getStatus(), PayTransferStatusEnum.IN_PROGRESS.getStatus()),
                new PayTransferDO().setStatus(PayTransferStatusEnum.SUCCESS.getStatus()).setSuccessTime(notify.getSuccessTime())
    ...
    private PayTransferExtensionDO updateTransferExtensionSuccess(PayTransferRespDTO notify) {
        ...
        if (transferExtension == null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_EXTENSION_NOT_FOUND);
        }
        if (PayTransferStatusEnum.isSuccess(transferExtension.getStatus())) { // 如果已成功，直接返回，不用重复更新
            log.info("[updateTransferExtensionSuccess][transferExtension({}) 已经是成功状态，无需更新]", transferExtension.getId());
            return transferExtension;
        }
        if (!PayTransferStatusEnum.isPendingStatus(transferExtension.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_EXTENSION_STATUS_IS_NOT_PENDING);
        }
        // 2. 更新 PayTransferExtensionDO
        int updateCount = transferExtensionMapper.updateByIdAndStatus(transferExtension.getId(),
                CollUtil.newArrayList(PayTransferStatusEnum.WAITING.getStatus(), PayTransferStatusEnum.IN_PROGRESS.getStatus()),
                new PayTransferExtensionDO().setStatus(PayTransferStatusEnum.SUCCESS.getStatus())
    ...
    private void updateTransferExtensionClosed(PayTransferRespDTO notify) {
        ...
        if (PayTransferStatusEnum.isClosed(transferExtension.getStatus())) { // 如果已是关闭状态，直接返回，不用重复更新
        ...
        int updateCount = transferExtensionMapper.updateByIdAndStatus(transferExtension.getId(),
                CollUtil.newArrayList(PayTransferStatusEnum.WAITING.getStatus(), PayTransferStatusEnum.IN_PROGRESS.getStatus()),
                new PayTransferExtensionDO().setStatus(PayTransferStatusEnum.CLOSED.getStatus())
                        .setChannelNotifyData(JsonUtils.toJsonString(notify)));
        if (updateCount == 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_EXTENSION_STATUS_IS_NOT_PENDING);
        }
        ...
        switch (transferType) {
            case ALIPAY_BALANCE: {
                ...
                if (!payChannel.getCode().startsWith("alipay")) {
                    throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_TYPE_AND_CHANNEL_NOT_MATCH);
                }
                break;
            }
            case WX_BALANCE:
    ...
    private PayTransferDO validateTransferCanSubmit(Long id) {
        ...
        if (PayTransferStatusEnum.isSuccess(transfer.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_STATUS_IS_SUCCESS);
        }
        if (!PayTransferStatusEnum.isWaiting(transfer.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_TRANSFER_STATUS_IS_NOT_WAITING);
        }
