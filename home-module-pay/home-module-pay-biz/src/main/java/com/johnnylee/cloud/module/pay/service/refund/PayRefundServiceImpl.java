...
import com.johnnylee.cloud.framework.tenant.core.util.TenantUtils;
...
public class PayRefundServiceImpl implements PayRefundService {
    ...
    private PayOrderDO validatePayOrderCanRefund(PayRefundCreateReqDTO reqDTO) {
        ...
        if (!PayOrderStatusEnum.isSuccessOrRefund(order.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.PAY_ORDER_REFUND_FAIL_STATUS_ERROR);
        }
        ...
        if (refundMapper.selectCountByAppIdAndOrderId(reqDTO.getAppId(), order.getId(),
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REFUND_HAS_REFUNDING);
        }
        return order;
    }
    ...
    private void notifyRefundSuccess(PayChannelDO channel, PayRefundRespDTO notify) {
        ...
        if (!PayRefundStatusEnum.WAITING.getStatus().equals(refund.getStatus())) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.REFUND_STATUS_IS_NOT_WAITING);
        }
        // 1.2 更新 PayRefundDO
