...
import com.johnnylee.cloud.framework.pay.core.enums.refund.PayRefundStatusRespEnum;
...
import com.johnnylee.cloud.module.pay.dal.mysql.wallet.PayWalletRechargeMapper;
...
import static com.johnnylee.cloud.framework.common.util.date.LocalDateTimeUtils.addTime;
import static com.johnnylee.cloud.framework.common.util.json.JsonUtils.toJsonString;
...
public class PayWalletRechargeServiceImpl implements PayWalletRechargeService {
    ...
    public PayWalletRechargeDO createWalletRecharge(Long userId, Integer userType, String userIp,
        ...
        if (Objects.isNull(reqVO.getPayPrice()) && Objects.isNull(reqVO.getPackageId())) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_PACKAGE_AND_PRICE_IS_EMPTY);
        }
        // 1.1 计算充值金额
        int payPrice;
        int bonusPrice = 0;
    ...
    public void updateWalletRechargerPaid(Long id, Long payOrderId) {
        ...
        if (walletRecharge == null) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_NOT_FOUND);
        }
        // 1.2 校验钱包充值是否可以支付
        ...
        if (updateCount == 0) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_UPDATE_PAID_STATUS_NOT_UNPAID);
        }
    ...
    public void refundWalletRecharge(Long id, String userIp) {
        ...
        walletRechargeMapper.updateById(new PayWalletRechargeDO().setPayRefundId(payRefundId)
                .setRefundStatus(PayRefundStatusEnum.WAITING.getStatus()).setId(walletRecharge.getId()));
    }
    ...
    public void updateWalletRechargeRefunded(Long id, Long payRefundId) {
        ...
        if (PayRefundStatusEnum.isSuccess(payRefund.getStatus())) {
            ...
            updateObj.setRefundStatus(PayRefundStatusEnum.SUCCESS.getStatus()).setRefundTime(payRefund.getSuccessTime())
        ...
        // 3. 更新钱包充值的退款字段
        walletRechargeMapper.updateByIdAndRefunded(id, PayRefundStatusEnum.WAITING.getStatus(), updateObj);
    }
    ...
    private PayRefundDO validateWalletRechargeCanRefunded(PayWalletRechargeDO walletRecharge, Long payRefundId) {
        ...
        if (notEqual(walletRecharge.getPayRefundId(), payRefundId)) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_REFUND_FAIL_REFUND_ORDER_ID_ERROR);
        }
        ...
        if (payRefund == null) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_REFUND_FAIL_REFUND_NOT_FOUND);
        }
        // 2.2 校验退款金额一致
        if (notEqual(payRefund.getRefundPrice(), walletRecharge.getPayPrice())) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_REFUND_FAIL_REFUND_PRICE_NOT_MATCH);
        }
        // 2.3 校验退款订单商户订单是否匹配
    ...
    private PayWalletDO validateWalletRechargeCanRefund(PayWalletRechargeDO walletRecharge) {
        ...
        if (!walletRecharge.getPayStatus()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_REFUND_FAIL_NOT_PAID);
        }
        // 校验充值订单是否已退款
        if (walletRecharge.getPayRefundId() != null) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_REFUND_FAIL_REFUNDED);
        }
        // 校验钱包余额是否足够
        ...
        if (wallet.getBalance() < walletRecharge.getTotalPrice()) {
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_REFUND_BALANCE_NOT_ENOUGH);
        }
    ...
    private PayOrderDO validateWalletRechargerCanPaid(PayWalletRechargeDO walletRecharge, Long payOrderId) {
        ...
        if (notEqual(walletRecharge.getPayOrderId(), payOrderId)) { // 支付单号
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_UPDATE_PAID_PAY_ORDER_ID_ERROR);
        }
        ...
        if (!PayOrderStatusEnum.isSuccess(payOrder.getStatus())) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_UPDATE_PAID_PAY_ORDER_STATUS_NOT_SUCCESS);
        }
        // 2.3 校验支付金额一致
        if (notEqual(payOrder.getPrice(), walletRecharge.getPayPrice())) {
            ...
            throw ServiceExceptionUtil.exception(ErrorCodeConstants.WALLET_RECHARGE_UPDATE_PAID_PAY_PRICE_NOT_MATCH);
        }
        // 2.4 校验支付订单的商户订单匹配
